...
export default function AdminOrdersScreen(): JSX.Element {
  ...
  const params = useLocalSearchParams<{ statusId?: string }>();
  ...
  // Sync statusId from URL query param if present
  useEffect(() => {
    if (params.statusId) {
      const parsed = parseInt(params.statusId, 10);
      if (!isNaN(parsed)) {
        setSelectedStatusId(parsed);
      }
    } else {
  ...
  const { data: stats, refetch: refetchStats, isRefetching: isRefetchingStats } = useAdminDashboardQuery();
  ...
  const isRefreshing = isRefetching || isRefetchingStats;
  ...
  // Backend returns filtered orders, no client-side filtering needed
  const filteredOrders = orders;

  // Dynamic order status count computation from dashboard stats
  const counts = useMemo(() => {
    return {
      all: stats?.totalOrders ?? 0,
      pendingReview: stats?.pendingOrdersCount ?? 0,
      processing: stats?.processingOrdersCount ?? 0,
      shipped: stats?.shippedOrdersCount ?? 0,
      delivered: stats?.deliveredOrdersCount ?? 0,
      cancelled: stats?.cancelledOrdersCount ?? 0,
    };
  }, [stats]);
    ...
    <View className="flex-1 bg-white">
      ...
      <View className="bg-[#0F4C92]" style={{ paddingTop: safeTop }}>
        <View className="flex-row-reverse items-center px-6 py-2.5">
          <Text className="text-lg font-bold text-white text-right">الطلبات</Text>
        </View>
      </View>
        ...
        { text: "تراجع", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              showSuccessToast("نجاح", "تم تحديث حالة الطلب بنجاح.");
            } catch (err: any) {
              showErrorToast("فشل التحديث", err.message || "حدث خطأ أثناء تعديل حالة الطلب.");
            }
          },
        },
      ]
          ...
          {isLoading ? (
            ...
            <>
              ...
              >
                ...
                <View className="bg-[#f8fafd] rounded-2xl p-4 border border-gray-100 mb-6">
                  <Text className="text-right font-black text-gray-800 text-xs mb-4">
                    خطوات حالة الطلب
                  </Text>
                  {currentStatusId === 5 ? (
                    <View className="flex-row-reverse items-center justify-center gap-2 bg-rose-50 border border-rose-100 p-3 rounded-xl">
                      <XCircle size={16} className="text-rose-600" />
                      <Text className="text-rose-700 font-bold text-xs">تم إلغاء هذا الطلب</Text>
                    </View>
                  ) : (
                    <View className="flex-row-reverse items-center justify-between px-1">
                      ...
                      <View className="items-center flex-1">
                        ...
                        >
                          {currentStatusId > 1 ? (
                            ...
                            <Text
                              className={`text-[10px] font-bold ${currentStatusId === 1 ? "text-white" : "text-gray-400"}`}
                            >
                              1
                            </Text>
                          )}
                        ...
                        <Text
                          className={`text-[9px] mt-1 font-bold text-center ${
                            currentStatusId === 1
                              ? "text-[#0F4C92]"
                              : currentStatusId > 1
                                ? "text-gray-700"
                      ...
                      {/* Line 1 */}
                      <View
                        className={`h-[2px] w-6 shrink-0 ${currentStatusId >= 2 ? "bg-[#0F4C92]" : "bg-gray-200"}`}
                      />
                      ...
                      <View className="items-center flex-1">
                        ...
                        >
                          {currentStatusId > 2 ? (
                            ...
                            <Text
                              className={`text-[10px] font-bold ${currentStatusId === 2 ? "text-white" : "text-gray-400"}`}
                            >
                              2
                            </Text>
                          )}
                      ...
                      {/* Line 2 */}
                      <View
                        className={`h-[2px] w-6 shrink-0 ${currentStatusId >= 3 ? "bg-[#0F4C92]" : "bg-gray-200"}`}
                      />
                      ...
                      <View className="items-center flex-1">
                        ...
                        >
                          {currentStatusId > 3 ? (
                            ...
                            <Text
                              className={`text-[10px] font-bold ${currentStatusId === 3 ? "text-white" : "text-gray-400"}`}
                            >
                              3
                            </Text>
                          )}
                      ...
                      {/* Line 3 */}
                      <View
                        className={`h-[2px] w-6 shrink-0 ${currentStatusId >= 4 ? "bg-[#0F4C92]" : "bg-gray-200"}`}
                      />
                      ...
                      <View className="items-center flex-1">
                        ...
                        >
                          {currentStatusId >= 4 ? (
                            ...
                            <Text className="text-[10px] font-bold text-gray-400">4</Text>
                          )}
                        </View>
                        <Text
                          className={`text-[9px] mt-1 font-bold text-center ${
                ...
                <View className="flex-row-reverse items-center justify-between mb-4">
                  <Text className="font-black text-gray-900 text-base">المنتجات المشتراة</Text>
                </View>
